...
export async function extract(
	...
	const routingInstruction = instructions.find(ix => {
		const decoded = parser.decodeInstruction(ix.data);
		return decoded.name !== "unknown" && parser.isRouting(decoded.name);
	});

	if (!routingInstruction) {
		console.log("No valid routing instruction found");
		return;
	}
...
/**
 * Units and decimals only. The parser used to price every leg here through
 * Jupiter's price API: up to three sequential HTTP calls per swap, priced at
 * parse time rather than trade time, and read by no consumer. Callers value
 * trades themselves.
 */
